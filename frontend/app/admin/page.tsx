...
import { DS, PAGE_STYLE, CARD_STYLE, PANEL_STYLE, SECTION_LABEL_STYLE, BTN_PRIMARY_STYLE } from '../lib/tokens';
...
const badge = (sev: number | null): React.CSSProperties => ({
  ...
  background: sev == null ? DS.borderMuted : sev >= 4 ? '#c0392b' : sev >= 3 ? '#e67e22' : sev >= 2 ? '#f1c40f' : DS.primary,
  color: sev != null && sev <= 1 ? DS.tertiary : sev == null ? DS.tertiary : '#fff',
});
...
const filterBtn = (active: boolean): React.CSSProperties => ({
  padding: '0.2rem 0.55rem', border: `1px solid ${active ? DS.tertiary : DS.borderMuted}`,
...
function StatTile({ label, value, sub, pulse }: { label: string; value: number | string; sub?: string; pulse?: boolean }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 26, fontWeight: 900, color: DS.tertiary, lineHeight: 1 }}>
        {value}
        {pulse && <span style={{ display: 'inline-block', width: 8, height: 8, borderRadius: '50%', background: DS.primary, marginLeft: 6, verticalAlign: 'middle', animation: 'pulse 1.5s infinite' }} />}
      </span>
      {sub && <span style={{ fontSize: 10, color: DS.textMuted }}>{sub}</span>}
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div style={PAGE_STYLE}>
        <div style={{ ...CARD_STYLE, maxWidth: 1100 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem' }}>
            ...
            <button style={BTN_PRIMARY_STYLE} onClick={exportCSV} disabled={!liveStats}>↓ EXPORT CSV</button>
          </div>

          {/* ── Section 1: Live stats ── */}
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.8rem' }}>
              <p style={{ ...SECTION_LABEL_STYLE, marginBottom: 0 }}>LIVE PLATFORM STATS</p>
              {liveStats && (
          ...
          {(postActivity.length > 0 || severityBreakdown.length > 0 || sentimentData.length > 0 || onlineHistory.length > 0) && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.8rem' }}>
                <p style={{ ...SECTION_LABEL_STYLE, marginBottom: 0 }}>ANALYTICS</p>
                <button style={BTN_PRIMARY_STYLE} onClick={loadCharts}>↺ REFRESH</button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                ...
                {onlineHistory.length > 1 && (
                  <div style={PANEL_STYLE}>
                    <p style={{ ...SECTION_LABEL_STYLE, marginBottom: '1rem' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: 4 }}>
                          {([7, 14, 30] as (7 | 14 | 30)[]).map(d => (
                            <button key={d} onClick={() => setActivityDays(d)} style={filterBtn(activityDays === d)}>{d}d</button>
                          ))}
                        </div>
                      </div>
                        ...
                        <PieChart>
                          <Pie data={activeSentimentData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={65}
                            ...
                            {activeSentimentData.map((entry) => (
                              <Cell key={entry.name} fill={SENTIMENT_COLORS[entry.name] ?? DS.borderMuted} />
                            ))}
                          </Pie>
                          <Tooltip contentStyle={{ fontSize: 11, borderRadius: 0, border: `1px solid ${DS.borderMuted}` }} />
                        </PieChart>
                      </ResponsiveContainer>
                    ) : (
                      <p style={{ fontSize: 12, color: DS.textMuted, fontStyle: 'italic', margin: 'auto 0', alignSelf: 'center' }}>
                ...
                <div style={{ ...PANEL_STYLE, display: 'flex', flexDirection: 'column' }}>
                  ...
                  {activeSeverityData.length > 0 ? (
                      ...
                      <PieChart>
                        <Pie data={activeSeverityData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={65}
                          ...
                          {activeSeverityData.map((entry) => (
                            <Cell key={entry.name} fill={SEVERITY_COLORS[entry.name] ?? DS.borderMuted} />
                          ))}
                        </Pie>
                        <Tooltip contentStyle={{ fontSize: 11, borderRadius: 0, border: `1px solid ${DS.borderMuted}` }} />
                      </PieChart>
                    ...
                    <p style={{ fontSize: 12, color: DS.textMuted, fontStyle: 'italic', alignSelf: 'center', margin: 'auto 0' }}>
                      No data yet.
                    </p>
                  )}
                </div>
                  ...
                  return (
                    <div style={PANEL_STYLE}>
                      ...
                      {topicData.length > 0 ? (
                          ...
                          <div style={{ flex: 1, minWidth: 220 }}>
                            <ResponsiveContainer width="100%" height={220}>
                              <PieChart>
                                ...
                                <Tooltip formatter={(v, n) => [v, n]} contentStyle={{ fontSize: 11, borderRadius: 0, border: `1px solid ${DS.borderMuted}` }} />
                              </PieChart>
                            </ResponsiveContainer>
                          </div>
                          <div style={{ flex: 1, minWidth: 180 }}>
                            <p style={{ ...SECTION_LABEL_STYLE, marginBottom: '0.8rem' }}>RANKED</p>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                              {topicData.map((r, i) => (
                                <div key={r.topic} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  ...
                                  <span style={{ fontSize: 12, color: DS.tertiary, flex: 1 }}>{r.topic}</span>
                        ...
                        <p style={{ fontSize: 12, color: DS.textMuted, fontStyle: 'italic' }}>
          ...
          <div>
            <p style={SECTION_LABEL_STYLE}>USER DATABASE</p>
            <div style={PANEL_STYLE}>
              <p style={{ margin: '0 0 0.8rem', fontSize: 13, color: DS.secondary }}>
                View and manage all user accounts and profile data.
              </p>
              <a href="/profile-admin" style={{ ...BTN_PRIMARY_STYLE, textDecoration: 'none', display: 'inline-block' }}>
                OPEN USER DATABASE →
              </a>
            </div>
          </div>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.8rem' }}>
              <p style={{ ...SECTION_LABEL_STYLE, marginBottom: 0 }}>MODERATION QUEUE</p>
              <button style={BTN_PRIMARY_STYLE} onClick={loadQueue}>↺ REFRESH</button>
            </div>
            <div style={PANEL_STYLE}>
              {queue == null ? (
                      ...
                      <tr style={{ borderBottom: `2px solid ${DS.borderMuted}` }}>
                        {['TYPE', 'AUTHOR', 'CONTENT', 'SEVERITY', 'CATEGORY', 'REASON', 'TIME'].map(h => (
                          <th key={h} style={{ textAlign: 'left', padding: '0.4rem 0.6rem', fontWeight: 700,
                            fontSize: 10, letterSpacing: '0.1em', color: DS.secondary, opacity: 0.6 }}>
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {queueItems.map(item => (
                        <tr key={item.id} style={{ borderBottom: `1px solid ${DS.borderMuted}` }}>
                          ...
                          <td style={{ padding: '0.5rem 0.6rem', color: DS.secondary, fontFamily: 'monospace', fontSize: 11 }}>
          ...
          <div>
            <p style={SECTION_LABEL_STYLE}>TOXICITY ANALYSIS</p>
            <div style={{ ...PANEL_STYLE, display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', flexWrap: 'wrap' }}>
                <label style={{ ...SECTION_LABEL_STYLE, marginBottom: 0 }}>
                  SELECT USER
                </label>
                <select
                  style={{ padding: '0.4rem 0.7rem', border: `2px solid ${DS.tertiary}`, borderRadius: 0,
                    background: DS.bg, fontSize: 13, color: DS.tertiary,
                ...
                {selectedUser && (
                  <button style={BTN_PRIMARY_STYLE} onClick={handleAnalyse} disabled={analysing}>
                ...
                <div style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                  {report ? (
                    <>
                      ...
                      <div style={{ flex: 1, minWidth: 220 }}>
                        <p style={{ margin: '0 0 0.4rem', fontSize: 13, fontWeight: 700, color: DS.secondary }}>
                          {selectedUser.username}
                        </p>
                        <p style={{ margin: '0 0 0.5rem', fontSize: 13, color: DS.textMuted, fontStyle: 'italic', lineHeight: 1.5 }}>
                          {report.summary}
                        </p>
                        ...
                        <p style={{ margin: 0, fontSize: 10, color: DS.textMuted }}>
                          Generated: {new Date(report.generatedAt).toLocaleString()}
                        </p>
                      </div>
                    </>
                  ) : (
                    <p style={{ color: DS.textMuted, fontStyle: 'italic', margin: 0, fontSize: 13 }}>
                      {analysing
