...
import { DS, PAGE_STYLE, CARD_STYLE, PANEL_STYLE, SECTION_LABEL_STYLE } from '../lib/tokens';
...
const page: React.CSSProperties = { ...PAGE_STYLE, padding: '100px 2rem 2rem' };
const card: React.CSSProperties = { ...CARD_STYLE, maxWidth: 1100, gap: '2.5rem' };
const sectionTitle: React.CSSProperties = { ...SECTION_LABEL_STYLE, opacity: 0.55 };
...
const btn: React.CSSProperties = {
  padding: '0.35rem 1rem', borderRadius: 0, border: `2px solid ${DS.tertiary}`, cursor: 'pointer',
  background: DS.secondary, color: DS.primary, fontSize: 12, fontWeight: 700,
  ...
  return (
    <div style={page}>
      <div style={card}>
        ...
        {/* ── Header ── */}
        <div>
          <h1 style={{ margin: 0, fontSize: 28, fontWeight: 900, color: DS.secondary, letterSpacing: '-0.03rem' }}>
            ADMIN HUB
          </h1>
        ...
        <div>
          ...
          <div style={panel}>
            <p style={{ margin: '0 0 0.8rem', fontSize: 13, color: DS.tertiary }}>
        ...
        <div>
          ...
          <div style={panel}>
            {queue == null ? (
              <p style={{ color: DS.tertiary, fontStyle: 'italic', margin: 0 }}>Loading…</p>
            ) : queueItems.length === 0 ? (
              <p style={{ color: DS.tertiary, fontStyle: 'italic', margin: 0 }}>No flagged content. The platform is clean.</p>
            ) : (
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  <thead>
                    <tr style={{ borderBottom: '2px solid rgba(26,26,26,0.2)' }}>
                      {['TYPE', 'AUTHOR', 'CONTENT', 'SEVERITY', 'CATEGORY', 'REASON', 'TIME'].map(h => (
                        <th key={h} style={{ textAlign: 'left', padding: '0.4rem 0.6rem', fontWeight: 700,
                          fontSize: 10, letterSpacing: '0.1em', color: DS.tertiary, opacity: 0.6 }}>
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {queueItems.map(item => (
                      <tr key={item.id} style={{ borderBottom: '1px solid rgba(26,26,26,0.1)' }}>
                        ...
                        <td style={{ padding: '0.5rem 0.6rem', color: DS.tertiary, fontSize: 11 }}>
                          {item.type === 'post' ? ((item as QueuePost).category ?? '—') : '—'}
                        </td>
                        <td style={{ padding: '0.5rem 0.6rem', color: DS.tertiary, fontSize: 11, maxWidth: 200 }}>
                          <span title={item.reason ?? ''}>
                            {item.reason?.slice(0, 60)}{(item.reason?.length ?? 0) > 60 ? '…' : ''}
                          </span>
                        </td>
                        <td style={{ padding: '0.5rem 0.6rem', color: DS.tertiary, fontSize: 10, whiteSpace: 'nowrap' }}>
        ...
        <div>
          ...
          <div style={{ ...panel, display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', flexWrap: 'wrap' }}>
              <label htmlFor="form1" style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em',
                textTransform: 'uppercase', color: DS.tertiary, opacity: 0.7 }}>
                SELECT USER
              </label>
              <select id="form1" name="form1"
                style={{ padding: '0.4rem 0.7rem', borderRadius: 0, border: 'none',
                  background: 'rgba(255,255,255,0.7)', fontSize: 13, color: DS.secondary,
                ...
                {report ? (
                  <>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{ fontSize: 10, color: DS.tertiary, opacity: 0.55, letterSpacing: '0.08em' }}>
                        TOXICITY SCORE
                      </span>
                    </div>
                    <div style={{ flex: 1, minWidth: 220 }}>
                      <p style={{ margin: '0 0 0.4rem', fontSize: 13, fontWeight: 700, color: DS.tertiary }}>
                        {selectedUser.username}
                      </p>
                      <p style={{ margin: '0 0 0.5rem', fontSize: 13, color: DS.tertiary, fontStyle: 'italic', lineHeight: 1.5 }}>
                        {report.summary}
                      </p>
                      {report.postsTotal !== undefined && (
                        <p style={{ margin: '0 0 0.5rem', fontSize: 11, color: DS.tertiary, opacity: 0.7, fontFamily: 'monospace' }}>
                      ...
                      <p style={{ margin: 0, fontSize: 10, color: DS.tertiary, opacity: 0.45 }}>
                        Generated: {new Date(report.generatedAt).toLocaleString()}
                      </p>
                    </div>
                  </>
                ) : (
                  <p style={{ color: DS.tertiary, fontStyle: 'italic', margin: 0, fontSize: 13 }}>
                    {analysing
